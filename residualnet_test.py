...
model = residualunet(input_shape) #/residualunet(input_shape)/attentionunet(input_shape)/attention_residualunet(input_shape) 

optimizer = Adam(learning_rate=0.001)

model.compile(optimizer = optimizer, loss= IoU_loss, metrics= ['accuracy', IoU_coef])
model.load_weights('./retina_resUnet.hdf5') #loading weights
